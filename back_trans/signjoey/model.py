...
from back_trans.signjoey.initialization import initialize_model
from back_trans.signjoey.embeddings import Embeddings, SpatialEmbeddings
from back_trans.signjoey.encoders import Encoder, RecurrentEncoder, TransformerEncoder
from back_trans.signjoey.decoders import Decoder, RecurrentDecoder, TransformerDecoder
from back_trans.signjoey.search import beam_search, greedy
from back_trans.signjoey.vocabulary import (
    TextVocabulary,
    GlossVocabulary,
    PAD_TOKEN,
    EOS_TOKEN,
...
from back_trans.signjoey.batch import Batch
from back_trans.signjoey.helpers import freeze_params
...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        self.sgn_embed_face = sgn_embed_face
    ...
    def forward(
        ...
        #encoder_output_face, encoder_hidden_face = self.encode_face(
        #   sgn=sgn_face, sgn_mask=sgn_mask_face, sgn_length=sgn_lengths
        #)

        #encoder_output = self.comb_layer(cat((encoder_output, encoder_output_face), -1))
    ...
    def encode_face(
        ...
        return self.face_encoder(
            embed_src=self.sgn_embed_face(x=sgn, mask=sgn_mask),
            src_length=sgn_length,
            mask=sgn_mask,
        )
    ...
    def get_loss_for_batch(
        ...
        decoder_outputs, gloss_probabilities = self.forward(
            ...
            sgn_face=batch.sgn_face,
            sgn_mask_face=batch.sgn_mask_face,
    ...
    def run_batch(
        ...
        #encoder_output_face, encoder_hidden_face = self.encode_face(
        #    sgn=batch.sgn_face, sgn_mask=batch.sgn_mask_face, sgn_length=batch.sgn_lengths
        #)
...
def build_model(
    ...
    sgn_embed_pose: SpatialEmbeddings = SpatialEmbeddings(
        **cfg["encoder"]["embeddings"],
        num_heads=cfg["encoder"]["num_heads"],
        input_size=sgn_dim,
    )
    sgn_embed_face: SpatialEmbeddings = SpatialEmbeddings(
        **cfg["encoder"]["embeddings"],
        num_heads=cfg["encoder"]["num_heads"],
        input_size=sgn_dim #-150,
    )
    sgn_embed_hands: SpatialEmbeddings = SpatialEmbeddings(
        **cfg["encoder"]["embeddings"],
        num_heads=cfg["encoder"]["num_heads"],
        input_size=sgn_dim,
    )
    sgn_embed_comb: SpatialEmbeddings = SpatialEmbeddings(
        **cfg["encoder"]["embeddings"],
        num_heads=cfg["encoder"]["num_heads"],
        input_size=sgn_dim,
    )
    ...
    if cfg["encoder"].get("type", "recurrent") == "transformer":
        ...
        pose_encoder = TransformerEncoder(
            **cfg["encoder"],
            emb_size=sgn_embed_pose.embedding_dim,
            emb_dropout=enc_emb_dropout,
        )
        face_encoder = TransformerEncoder(
            **cfg["encoder"],
            emb_size=sgn_embed_face.embedding_dim,
            emb_dropout=enc_emb_dropout,
        )
        hands_encoder = TransformerEncoder(
            **cfg["encoder"],
            emb_size=sgn_embed_hands.embedding_dim,
            emb_dropout=enc_emb_dropout,
        )

        comb_layer = nn.Linear(1024, 512)
    else:
    ...
    if do_recognition:
        gloss_output_layer = nn.Linear(pose_encoder.output_size, len(gls_vocab))
    ...
    model: SignModel = SignModel(
        ...
        comb_layer=comb_layer,
        gloss_output_layer=gloss_output_layer,
        decoder=decoder,
        sgn_embed=sgn_embed_pose,
        sgn_embed_face=sgn_embed_face,
        txt_embed=txt_embed,
        gls_vocab=gls_vocab,
        txt_vocab=txt_vocab,
