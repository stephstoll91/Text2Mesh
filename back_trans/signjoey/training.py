...
from back_trans.signjoey.model import build_model
from back_trans.signjoey.batch import Batch
from back_trans.signjoey.helpers import (
    log_data_info,
    load_config,
    log_cfg,
    load_checkpoint,
...
from back_trans.signjoey.model import SignModel
from back_trans.signjoey.prediction import validate_on_data
from back_trans.signjoey.loss import XentLoss
from back_trans.signjoey.data import load_data, make_data_iter
from back_trans.signjoey.builders import build_optimizer, build_scheduler, build_gradient_clipper
from back_trans.signjoey.prediction import test
from back_trans.signjoey.metrics import wer_single
from back_trans.signjoey.vocabulary import SIL_TOKEN, build_vocab
...
def build_model_for_backtrans(cfg):
    _, _, _, gls_vocab, txt_vocab = load_data(
        data_cfg=cfg["data"]
    )
...
def build_inference_model_for_backtrans(cfg):

    data_cfg = cfg['data']
    gls_vocab_file = os.path.join(cfg['training']['model_dir'], data_cfg.get("gls_vocab", None))
    txt_vocab_file = os.path.join(cfg['training']['model_dir'], data_cfg.get("txt_vocab", None))
