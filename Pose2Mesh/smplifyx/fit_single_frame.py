...
from Pose2Mesh.smplifyx.optimizers import optim_factory

import Pose2Mesh.smplifyx.fitting as fitting

import human_body_prior.tools.model_loader as hpm
from human_body_prior.models.vposer_model import VPoser

from Pose2Mesh.smplifyx.raymond_lights import create_raymond_lights
...
def fit_single_frame(img,
    ...
    if idx <=0:
        ...
        if use_face:
            ...
            opt_weights_dict['jaw_prior_weight'] = [[4040.0, 40400.0, 40400.0],
                                                    ...
                                                    [470.8, 4708.0, 4708.0]]  # jaw_pose_prior_weights
        if use_hands:
            opt_weights_dict['hand_weight'] = [0.0, 0.0, 0.0, 0.1, 5.0] #hand_joints_weights
            opt_weights_dict['hand_prior_weight'] = [404.0, 404.0, 57.4, 30.0, 0.1]  # hand_pose_prior_weights
        if interpenetration:
            opt_weights_dict['coll_loss_weight'] = coll_loss_weights
    ...
    with fitting.FittingMonitor(
        ...
        # body_model.global_orient = torch.nn.Parameter(torch.tensor([[0., 2.75700693, 1.50615975]]).to(device))
        ...
        for or_idx, orient in enumerate(tqdm(orientations, desc='Orientation')):
            ...
            for opt_idx, curr_weights in enumerate(tqdm(opt_weights, desc='Stage')):
                ...
                closure = monitor.create_fitting_closure(
                    ...
                    use_vposer=use_vposer, vposer=vposer,
                    pose_embedding=pose_embedding,
                    return_verts=True, return_full_pose=True, out_model_t=j_diff)
            ...
            if interactive and final_loss_val:
                if use_cuda and torch.cuda.is_available():
                    torch.cuda.synchronize()
    ...
    if save_meshes or visualize:
        body_pose = vposer.decode(
            pose_embedding)['pose_body'].contiguous().view(-1, 63) if use_vposer else None
        ...
        if j_diff is not None:
            projected_joints = camera(model_output.joints).cpu().detach().numpy()
            projected_joints_t = camera(j_diff.joints).cpu().detach().numpy()
            t_diff = np.abs(np.mean(projected_joints - projected_joints_t))
        else:
            t_diff = 0.0

    return body_model, pose_embedding, body_mean_pose, camera, joints, joints_gt, model_output, t_diff
