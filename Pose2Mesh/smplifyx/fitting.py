...
from Pose2Mesh.smplifyx.mesh_viewer import MeshViewer
import Pose2Mesh.smplifyx.utils as utils
...
def guess_init(model,
    ...
    #j2d = joints_2d.cpu().detach().numpy()
    #j3d = joints_3d.cpu().detach().numpy()
    #plt.scatter(j2d[:, :, 0], j2d[:, :, 1])
    #plt.show()
...
class FittingMonitor(object):
        ...
        for n in range(self.maxiters):
            ...
            try:
                if torch.isnan(loss).sum() > 0:
                    raise Exception('NaN loss value, stopping!')
            except Exception as e:
                print(e)
                return None
            try:
                if torch.isinf(loss).sum() > 0:
                    raise Exception('Infinite loss value, stopping!')
            except Exception as e:
                print(e)
                return None
    ...
    def create_fitting_closure(self,
                               ...
                               pose_embedding=None,
                               create_graph=False, out_model_t=None,
                               **kwargs):
...
class SMPLifyLoss(nn.Module):
    ...
    def forward(self, body_model_output, camera, gt_joints, joints_conf,
                ...
                use_vposer=False, pose_embedding=None, out_model_t=None,
                **kwargs):
        projected_joints = camera(body_model_output.joints)
        if out_model_t is not None:
            projected_joints_t = camera(out_model_t.joints)
        ...
        if out_model_t is not None:
            joint_diff_t = self.robustifier(projected_joints_t - projected_joints)
            joint_loss_t = (torch.sum(weights ** 2 * joint_diff_t) *
                            self.data_weight ** 2)
        else:
            joint_loss_t = 0.0
        joint_loss = (torch.sum(weights ** 2 * joint_diff) *
                      self.data_weight ** 2) + 0.1 * joint_loss_t
